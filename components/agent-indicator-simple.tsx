...
export function AgentIndicatorSimple({ currentAgent, isTransitioning, transitionMessage }: AgentIndicatorProps) {
        ...
        {isTransitioning && transitionMessage ? (
          ...
          >
            <div className={`flex items-center space-x-3 p-3 rounded-lg ${config.bgColor} ${config.borderColor} border`}>
              <div className={`p-2 rounded-full bg-gradient-to-r ${config.color} text-white flex items-center justify-center text-lg`}>
                {config.icon}
              </div>
